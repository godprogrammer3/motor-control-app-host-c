<template>
  <div>
    <v-app-bar color="indigo darken-4" style="height:70px" flat>
      ...
      <span class="white--text text-h4 ml-5">({{ currentJobOrder + 1 }}/{{ group.job.length }} งาน)</span>
    </v-app-bar>
    <v-container fluid class="pa-0">
      <v-row class="elevation-2" style="background-color:white;" align="center" justify="center">
        ...
        <v-col align="center" justify="center"><span class="indigo--text text-h5">หน้ากว้าง :</span><span
            class="text-h5 ml-2">
            {{ group.job[currentJobOrder].width }} นิ้ว</span></v-col>
      ...
      <footer v-if="currentJobOrder + 1 < group.job.length" class="elevation-2 pl-10" fixed absolute>
      ...
      <v-row align="center" justify="center" style="height:10vh;">
        <v-col justify="center" style="width:100%;">
          <v-row align="center" justify="center">
            <v-switch style="transform:scale(1.3);" v-model="isAutoMode" inset color="green">
              <template v-slot:label>
                <span :class="(isAutoMode)?'green--text':'orange--text'">{{isAutoMode ? 'โหมดอัตโนมัติ' :
                  'โหมดแมนนวล'}}</span>
                <v-icon style="margin-left:5px;transform:scale(1.2);" x-large :color="isAutoMode?'green':'orange'">
        ...
        <v-col align="center" justify="center">
          <v-btn color="indigo" dark x-large bottom @click="addWastPaper" class="text-h5">
            เพิ่มกระดาษเสีย
          </v-btn>
        </v-col>
      </v-row>
